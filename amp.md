...
## 一、半精度浮点类型 FP16
首先介绍半精度（FP16）。如下图所示，半精度浮点数是一种相对较新的浮点类型，在计算机中使用2字节（16比特）存储。在IEEE 754-2008标准中，它亦被称作binary16。FP32 是单精度浮点数，用8bit 表示指数，23bit 表示小数；FP16半精度浮点数，用5bit 表示指数，10bit 表示小数。FP16 的动态范围（6*1E-8 ~ 65504）远低于 FP32 的动态范围（1.4*1E-45 ~ 1.7*1E+38），并且 FP16 的精度（2-10）远低于 FP32 的精度（2-23），因此FP16更适于在精度要求不高的场景中使用。  
<div  align="center">  
